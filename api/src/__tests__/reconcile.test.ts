...
describe("delivery reconciliation", () => {
  ...
  it("rebuilds the next durable attempt with its remaining delay", async () => {
    ...
    expect(mocks.enqueueDelivery).toHaveBeenCalledWith("delivery_1", 3, 20_000);
  ...
  it("continues repairing other deliveries when one projection fails", async () => {
    mocks.findMany.mockResolvedValue([
      { id: "delivery_1", attemptCount: 0, nextAttemptAt: null },
      { id: "delivery_2", attemptCount: 1, nextAttemptAt: null },
    ]);
    ...
    expect(mocks.enqueueDelivery).toHaveBeenNthCalledWith(1, "delivery_1", 1, 0);
    expect(mocks.enqueueDelivery).toHaveBeenNthCalledWith(2, "delivery_2", 2, 0);
